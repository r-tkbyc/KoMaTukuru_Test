...
> **AIエージェント向け補足**: このセクションは v2.1.0 で追加され、v2.1.4 でスコアリングロジックが強化されました。`index.html` 内の PDF 取り込みブロック（`#pdfImportPanel`）と、末尾の `<script>` ブロック（PDF.js ローダー）がこれに対応します。仕様追加・修正時はこのセクションとコードの両方を更新してください。
...
### 8.2 抽出方式（2段階フォールバック）
1. **フォームフィールド優先**  
   PDF のフォームフィールド（入力可能なテキストボックス）を検索し、フィールド名およびフィールド値の特徴量スコアから4項目を分類します。フィールドが存在し、かつ分類に成功した場合はこちらを採用します。
2. **テキストレイヤーフォールバック**  
...
### 8.3 フォームフィールド方式のスコアリング設計

各フィールドに対して `scoreKaiki` / `scoreVenue` / `scoreTitle` / `scoreBody` の4スコアを計算し、スコアの高い順に割り当てる（`kaiki → venue → honbun → title` の順で先取り）。

**`scoreTitle` の陽性シグナル（v2.1.4 強化）**

| 条件 | 加点 |
|---|---|
| `fieldName` が `title / タイトル / 題名` に一致 | +6 |
| 文字数が 2〜90 字 | +6 |
| 1行テキスト | +3 |
| `■` で始まらない | +2 |
| 日付を含まない | +2 |
...
> `fieldName` バイアスは他カテゴリにも同様に追加されています（`kaiki / 会期 / period` → `scoreKaiki` +6 など）。
