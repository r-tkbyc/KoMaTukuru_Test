...
### 6.1 `<head>` に追記
```html```
<!-- Text Formatter v2.1.0 - 2026-03-03 -->
...
> **AIエージェント向け補足**: このセクションは v2.1.0 で追加された機能です。`index.html` 内の PDF 取り込みブロック（`#pdfImportPanel`）と、末尾の `<script>` ブロック（PDF.js ローダー）がこれに対応します。仕様追加・修正時はこのセクションとコードの両方を更新してください。
...
### 8.2 抽出方式（2段階フォールバック）
1. **フォームフィールド優先**  
   PDF のフォームフィールド（入力可能なテキストボックス）を検索し、フィールド名から4項目を分類します。フィールドが存在し、かつ分類に成功した場合はこちらを採用します。
2. **テキストレイヤーフォールバック**  
